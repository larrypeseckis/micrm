# MICRM-v0.2: Machine-Initiated Consequence Reference Model

*Author: Larry Peseckis | larrypeseckis.ai*  
*Version: v0.2 | Dated: 2026-07-16*  
*Status: Canonical at the witnessed repository commit. Not for public distribution.*  
*Canonical predecessor: MICRM-v0, file dated 2026-07-11; witnessed repository timestamp 2026-07-12.*

> **Canonicalization notice:** This document is the canonical successor to MICRM-v0. It derives from author-ratified full-draft state SHA-256 `0db60d0a240361d410ac2370a040fbadf284a203d9b7b22e19f7cc795897f62f` (repository provenance commit `5115be08a4b2ff5bff7b7699519199de659efeeb`), repaired through reviewer-verified patches `e848e08e` and `750711af` to reviewer-ratified state SHA-256 `47baffc22f90c92065abaac649a02fc6c1d23bcd406156f72e10ce5a982de911`, then canonicalized by the status-only patch authorized in Larry Peseckis's ratification declaration. Canonical effect begins at the witnessed repository commit of this file. The two scope-assumption clauses are declared assumptions, not unresolved architecture.
...
[ASSUME: The run graph is resolved at a level sufficient to distinguish the traversals and consequence events relevant to the incident. Where resolution is uncertain, the uncertainty is recorded and cannot support an exit from accountability.]
...
**Separately committed admission result:** an independently identifiable change in state, authority, routing, or availability that persists beyond the admission mechanism's internal evaluation and is available to become Context, an input to another traversal, or a consequence represented in the run graph.

Resolve a borderline admission mechanism in two stages. First determine whether a separately committed admission result is established, excluded, or unresolved. If it is excluded, use the embedded Context-boundary representation. If it is established, use the embedded representation only when accountability equivalence is established; promote the mechanism when inequivalence is established; and retain an admission-resolution fork when equivalence is unresolved. If existence of the separate result is unresolved, retain both representations only where both remain evidence-compatible. A fork does not establish that the promoted traversal occurred.

| Separate-result finding | Accountability-equivalence finding | Representation result |
|---|---|---|
| Excluded | Not applicable | Embedded representation |
| Established | Equivalent | Embedded representation permitted |
| Established | Inequivalent | Promoted traversal required |
| Established | Unresolved | Admission-resolution fork |
| Unresolved | Not applicable pending separate-result resolution | Fork only while both representations remain evidence-compatible |

Under an admission-resolution fork, the first invariant, candidate sets, and exits are evaluated separately under each evidence-compatible representation. A root exits only if it establishes exit in every evidence-compatible representation in which it is a candidate. Uncertainty about the representation cannot support exit.

If exactly one representation remains evidence-compatible, the separate-result finding must be re-evaluated as established or excluded before that representation is selected. If the finding cannot be reclassified, graph resolution remains unresolved and cannot support exit. If neither representation remains evidence-compatible, the incident graph requires further resolution and neither representation may be treated as established.
...
1. **Path continuity:** every required channel or state reaches the proposed composition locus on the incident graph with its required capacity intact. Independently established path absence, rejection, replacement, supersession, or neutralization upstream of the composition locus is applied. A candidate propagation cut or structural exclusion whose validity depends on this composition finding is not presumed.
...
For a residual dark surface, every incident-relevant class of finite indexed collections admitted by the bound must be excluded, either by evaluating its members or by independently supported structural proof that applies to the class. A collection blocks exit when composition is present or unresolved. Capacity exclusion is established only when every admitted incident-relevant class is excluded. The rule does not require total observability; it requires resolution only of the specific composition pathways relevant to the alleged contribution class.
...
7. **Causal supersession or Gate-stage neutralization:** at final Gate outcome, no upstream-originated state or capacity attributable to the exiting root retained operative causal capacity for the alleged contribution class, either as a complete mechanism or as an indexed component of a proposed distributed contribution mechanism whose composition is present or unresolved under Section 7.2. Where a composition is present, each indexed component prevents the root's cut unless superseded or neutralized; where composition is unresolved, capacity exclusion is not established and the cut fails by burden; where a proposed composition is excluded, that composition does not prevent the cut. This may be established because the relevant contribution-bearing capacity originated downstream of the seam, or because every upstream-originated capacity attributable to the root had been structurally superseded or positively neutralized no later than final Gate outcome. Neutralization completed after final Gate outcome is evaluated as structural exclusion under Section 9.4 and cannot complete a qualifying propagation cut.
...
**Operative causal capacity** is the incident-specific capability of a state or transition to enable, disable, authorize, parameterize, redirect, amplify, suppress, or otherwise alter a consequence-relevant transition through a directed, type-compatible mechanism leading to the selected consequence, where no qualifying propagation cut, structural exclusion, or neutralization upstream of the evaluation point has already removed the required capacity. The candidate cut or exclusion being evaluated is not presumed.
...
Cross-root seam record:
  ...
  handoff state:
    ...
    contribution-capacity profile:
    distributed mechanism record link, if any:
    component index, if any:

  admission and Context formation:
    admission controls:
    accepted / rejected / transformed / quarantined:
    admission-resolution record, if borderline:
      embedded representation graph reference:
      promoted representation graph reference:
      separate-result existence finding
        (established / excluded / unresolved):
      separately committed result evidence:
      accountability-equivalence finding
        (equivalent / inequivalent / unresolved / not applicable):
      resolution finding
        (embedded / promoted / fork unresolved):
...
### 11.1 Run-level distributed contribution mechanism record

A distributed composition determination spans roots and seams. Recording the final finding independently inside each participating seam would permit inconsistent copies of one run-level determination. The run-level record below is the authoritative composition finding; participating seams link to it through the distributed mechanism record link and component index fields and remain authoritative only for their local handoff evidence.

```text
distributed contribution mechanism record, repeatable:
  mechanism identifier:
  selected consequence identifier:
  alleged contribution class identifier:
  indexed components, repeatable:
    component index:
    originating root:
    traversal identifier:
    path identifier:
    seam identifier:
    contribution-capacity profile:
    transformation finding, if applicable:
  composition locus or ordered subgraph:
  mechanism-specific composition rule or operator:
  path-continuity finding:
  path-continuity evidence:
  incident-association finding:
  incident-association evidence:
  temporal-compatibility finding:
  temporal-compatibility evidence:
  composition-semantics finding:
  composition-semantics evidence:
  type-composition finding:
  type-composition evidence:
  consequence-reach finding:
  consequence-reach evidence:
  final composition finding
    (present / excluded / unresolved):
  independent corroboration:
  custody and modification control:
  residual dark-surface bound:
```
...
These examples illustrate the v0.2 mechanics and are not a substitute for incident-specific evidence. They formed part of author-ratified source state SHA-256 `0db60d0a240361d410ac2370a040fbadf284a203d9b7b22e19f7cc795897f62f`. Wording changed by the post-ratification repair patches was subsequently reviewer-ratified and author-ratified in state SHA-256 `47baffc22f90c92065abaac649a02fc6c1d23bcd406156f72e10ce5a982de911`.
...
- C's queue placement is a separately committed admission result: the placement persists beyond the mechanism's internal evaluation and becomes Context for B, so the separate result is established. Embedding the service only inside B's Context would hide C's bonded root, a separate consequence, and a path-level control role, so the representations are not accountability-equivalent and C's queue-placement decision is promoted to a traversal;
- a passive format check inside B that emits evidence but commits no independently identifiable change in state, authority, routing, or availability has its separate result excluded and remains a Context-boundary control without requiring an equivalence finding;
- and where the separate result is established but accountability equivalence is unresolved, or the separate result's existence is unresolved while both representations remain evidence-compatible, the record retains an admission-resolution fork and an exit must hold under every evidence-compatible representation in which the root is a candidate.
...
4. **Keep-in-pocket architecture:** tiering, irreversibility, two-ledger residual, mount-point regime, and bonded-root economics remain outside v0.2 unless separately ratified.
...
The following emerged through the joint-accountability dialogue between Larry Peseckis and AI collaborators. Architectural content developed through 2026-07-15 was ratified for inclusion in the prior review state. The six uncertainty resolutions added on 2026-07-16 were author-ratified as part of full-draft state SHA-256 `0db60d0a240361d410ac2370a040fbadf284a203d9b7b22e19f7cc795897f62f`. Wording altered by the subsequent repair patches was reviewer-ratified and author-ratified in state SHA-256 `47baffc22f90c92065abaac649a02fc6c1d23bcd406156f72e10ce5a982de911`.
...
The following were constructed in dialogue at Larry's direction on 2026-07-16 and are included in this version at dialogue-level attribution:
...
These resolutions replaced the corresponding deferrals in the prior review state and were author-ratified in source state SHA-256 `0db60d0a240361d410ac2370a040fbadf284a203d9b7b22e19f7cc795897f62f`. The post-ratification repair wording was subsequently reviewer-ratified and author-ratified in state SHA-256 `47baffc22f90c92065abaac649a02fc6c1d23bcd406156f72e10ce5a982de911`.
...
- downstream "reliance" as the authority-representation discriminator;
- record silence treated as an exhaustive two-cause system of residual dark surface versus non-emission;
- and the categorical claim that intervention-selective records can support binding but cannot support exit.

### 17.6 Version status

This provenance appendix records the development history represented by this version. Larry Peseckis ratified full-draft state SHA-256 `0db60d0a240361d410ac2370a040fbadf284a203d9b7b22e19f7cc795897f62f`. That state had previously entered the repository as the explicitly non-canonical provenance commit `5115be08a4b2ff5bff7b7699519199de659efeeb`. The post-ratification reviewer repairs differed from the ratified state and required separate ratification; author ratification of the `0db60d0a` state remains a valid provenance fact and is not erased by later defects or repairs. The earlier reviewer ratification of review-draft SHA-256 `a493d4793431cd31b0772a05a60e2856f1af055e464f43ab216eb4ab67dc988b` covered that byte state and did not extend to the substantive uncertainty resolutions added afterward. Reviewer ratification was subsequently granted for repaired state SHA-256 `47baffc22f90c92065abaac649a02fc6c1d23bcd406156f72e10ce5a982de911`. Larry Peseckis ratified that exact state and authorized the status-only canonicalization patch producing this document. Canonical effect begins at the witnessed repository commit of this file.
...
*MICRM-v0.2, canonical successor to MICRM-v0. Compiled from MICRM-v0 and joint-accountability development through 2026-07-16. All prior explicit uncertainty markers are resolved in the text. Author-ratified and reviewer-ratified; canonical at the witnessed repository commit.*
